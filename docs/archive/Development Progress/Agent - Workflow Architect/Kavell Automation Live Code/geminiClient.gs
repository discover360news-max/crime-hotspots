...
function buildExtractionPrompt(articleText, articleTitle, publishedDate) {
    ...
    "crimes": [
      {
        ...
        "crime_type": "Murder|Shooting|Robbery|Assault|Theft|Home Invasion|Sexual Assault|Kidnapping|Other",
  ...
  RULES:
  - Published ${pubDateStr}. Calculate crime dates from this.
  - "yesterday" = ${pubDateStr} minus 1 day
  - "Monday" = calculate from ${pubDateStr}
  - If no date, use ${pubDateStr}
  - Not a crime article? Return {"crimes": [], "confidence": 0}
  - One article, multiple crimes? Create separate crime objects
