// src/js/dashboardBarbados.js
// Barbados dashboard using modular components

import { createBarbadosMap } from './components/barbadosMap.js';
import { fetchBarbadosData, calculateStats } from './services/barbadosDataService.js';
import {
  createMetricsCards,
  createPieChart,
...
async function init() {
  try {
    ...
    if (mapContainer) {
      try {
        barbadosMapInstance = createBarbadosMap(handleRegionClick);
        ...
        mapContainer.appendChild(barbadosMapInstance.element);
      } catch (error) {
    ...
    regionTray = new RegionTray({
      ...
      createMapFn: (onClickCallback) => {
        return createBarbadosMap((regionName) => {
          handleRegionClick(regionName);
          onClickCallback(regionName);
        });
      },
      onReset: async () => {
        await handleRegionClick(null);
        if (filterController) {
    ...
    document.addEventListener('keydown', (e) => {
      if (e.key === 'r' || e.key === 'R') {
        if (filterController && filterController.getRegion() && barbadosMapInstance) {
          e.preventDefault();
          barbadosMapInstance.clearFilter();
...
async function loadDashboard(regionFilter = null, dateRange = null) {
  ...
  try {
    ...
    if (!allData) {
      console.log('Fetching Barbados data from CSV...');
      allData = await fetchBarbadosData();
  ...
  try {
    ...
    if (allData) {
      try {
        ...
        leafletMapInstance = createBarbadosLeafletMap(filteredMapData, currentRegion?.name);
...
async function handleRegionClick(regionName) {
  ...
  try {
    if (regionName) {
      // Parish selected - update filter controller
      if (filterController) {
        filterController.setRegion({ name: regionName });
      }
      ...
      if (barbadosMapInstance && typeof barbadosMapInstance.clearFilter === 'function') {
        try {
          barbadosMapInstance.clearFilter();
        } catch (error) {
...
function updateSubtitle(region, dateRange, errorText = null) {
  ...
  if (region) {
    subtitleText = `Region ${region.number} - ${region.name}`;
  } else {
