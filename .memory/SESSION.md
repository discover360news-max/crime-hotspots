...
- **Next:** JNews hierarchy rollout complete across all remaining pages.

## Session Notes — Mar 25, 2026
- **`murders.astro` JNews hierarchy** — dark hero (H1 = murder count as big number, subtitle = "Murders in Trinidad & Tobago · {year}", live pulse = YoY same-period comparison sentence, CTAs: Murder Count/Statistics, freshness line). 4-card vitals: Murders YTD (crimson), Projected Annual (amber), vs Last Year % (violet), Days with Murders (slate). Added vars: `daysPassed`, `annualizedRate`, `prevYearSamePeriodCount`, `yoyDisplay`, `yoyPulseText`. `Breadcrumbs.astro` removed (breadcrumb now inline in hero); `GradientStatCard` added. `max-w-3xl` → `max-w-5xl`. Date group `<h2>` → `<h3>` (proper hierarchy under new section `<h2>`). 237 → 303 lines. Build passes.
- **`crime/[slug].astro` JNews article layout** — removed single `rounded-3xl shadow-md` card wrapper. New structure: category pill above H1 (crime type in rose, related types in slate), `font-black` H1, byline row (date · area · source). Featured crime type thumbnail image (`h-40 sm:h-52 object-cover rounded-xl`). Slim nav row (Dashboard/Headlines/Safety Tips). `lg:grid-cols-[1fr_256px]` main+sidebar: main has summary → report issue → SafetyContext → tips → feedback → TrendingHotspots → prev/next; sidebar has share card (3 buttons) → RelatedCrimes → support card. RelatedCrimes consolidated (was separate mobile/desktop blocks). Share script wrapped in `DOMContentLoaded`. `max-w-3xl` → `max-w-4xl`. `image` field added to NewsArticle schema. 488 → 483 lines. Build passes.
- **Next:** `regions.astro` JNews hierarchy (MEDIUM) — compact dark hero only, no vitals row. Then `compare.astro` hero swap (rose → dark slate).
...
- **Next:** `murders.astro` JNews hierarchy (MEDIUM) — dark hero + flip counter as vitals. ✓ DONE
