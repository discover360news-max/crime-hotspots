...
export function createHeadlineSummaryModal() {
      ...
      <div class="fixed bottom-0 left-0 right-0 max-h-[90vh] overflow-hidden transform transition-all duration-500 translate-y-full" id="modalContent">
        ...
        <div class="bg-white/80 backdrop-blur-lg shadow-2xl rounded-t-2xl overflow-hidden">
          ...
          <div class="flex items-center justify-between px-4 py-3 border-b border-slate-200">
            <h3 class="text-small font-semibold text-slate-700">Incident Summary</h3>
          ...
          <div class="overflow-y-auto max-h-[calc(90vh-4rem)] p-4">
            ...
            <div class="flex flex-wrap gap-2 mb-3">
              <span id="crimeTypeBadge" class="px-2 py-0.5 bg-rose-100 text-rose-700 text-tiny font-semibold rounded-md uppercase"></span>
              <span id="dateBadge" class="px-2 py-0.5 bg-slate-100 text-slate-600 text-tiny font-medium rounded-md"></span>
            </div>

            <!-- Headline -->
            <h2 id="summaryHeadline" class="text-h2 font-bold text-slate-900 mb-3 leading-tight"></h2>

            <!-- Summary text -->
            <div id="summaryText" class="text-body max-w-none mb-4 text-slate-700 leading-relaxed"></div>

            <!-- Location -->
            <div id="locationSection" class="flex items-start gap-2 mb-4 p-3 bg-slate-50/50 rounded-lg">
              ...
              <div class="flex-1">
                <div id="locationText" class="text-tiny text-slate-700"></div>
              </div>
            </div>

            <!-- Source attribution -->
            <div id="sourceSection" class="mb-4 p-3 bg-slate-50/50 rounded-lg border-l-2 border-slate-300">
              <p class="text-tiny text-slate-600 mb-1">Source</p>
              <a id="sourceLink" href="#" target="_blank" rel="noopener noreferrer" class="text-tiny text-slate-900 hover:text-rose-600 font-medium underline"></a>
            </div>

            <!-- Action buttons -->
            <div class="flex flex-wrap gap-2 mb-4">
              <a id="readFullStoryBtn" href="#" target="_blank" rel="noopener noreferrer nofollow" class="px-3 py-1.5 border-2 border-rose-600 text-rose-600 rounded-lg hover:bg-rose-50 transition text-tiny font-medium flex items-center gap-1">
                <span>Read Full Story</span>
                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" />
                </svg>
              </a>
              <button id="viewOnMapBtn" class="px-3 py-1.5 border-2 border-rose-600 text-rose-600 rounded-lg hover:bg-rose-50 transition text-tiny font-medium flex items-center gap-1">
                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <!-- Report Issue -->
            <div class="pt-4 border-t border-slate-200/50">
              <button id="reportIssueToggle" class="text-tiny text-slate-500 hover:text-rose-600 font-medium flex items-center gap-1">
                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  function show(crime, countrySlug = null) {
    ...
    // Source
    const sourceName = getSourceName(crime.URL);
    const sourceLink = document.getElementById('sourceLink');
    sourceLink.textContent = sourceName;
    sourceLink.href = crime.URL || '#';

    // Read full story button
    readFullStoryBtn.href = crime.URL || '#';
    if (!crime.URL) {
      readFullStoryBtn.classList.add('opacity-50', 'cursor-not-allowed');
      readFullStoryBtn.setAttribute('disabled', 'true');
    } else {
      readFullStoryBtn.classList.remove('opacity-50', 'cursor-not-allowed');
      readFullStoryBtn.removeAttribute('disabled');
    }
