...
// Guyana dashboard using modular components

import { createGuyanaMap } from './components/guyanaMap.js';
  ...
  try {
    ...
    regionTray = new RegionTray({
      ...
      createMapFn: (onClickCallback) => {
        return createGuyanaMap((regionNumber, regionName) => {
    ...
    document.addEventListener('keydown', (e) => {
      if (e.key === 'r' || e.key === 'R') {
        if (filterController && filterController.getRegion() && guyanaMapInstance) {
          e.preventDefault();
  ...
  try {
    ...
    if (allData) {
      try {
        ...
        leafletMapInstance = createGuyanaLeafletMap(filteredMapData, currentRegion?.name);
