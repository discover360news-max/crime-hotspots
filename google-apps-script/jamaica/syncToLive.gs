/**
 * Production → LIVE Sync Helper for Jamaica
 *
 * Purpose: Copy approved crimes from Production sheet to a separate LIVE spreadsheet.
 * The LIVE sheet is published as CSV — that URL is what the D1 sync worker fetches.
 *
 * SETUP (do once before first run):
 *   1. Create a new Google Sheet: "Crime Hotspots - Jamaica LIVE"
 *   2. Create a tab named "LIVE" with the same column headers as Production
 *   3. File → Share → Publish to web → select LIVE tab → CSV → copy the URL
 *   4. Paste the spreadsheet ID into LIVE_SHEET_ID below
 *   5. Update workers/jamaica-crime-sync/wrangler.toml with the CSV URL
 *
 * Features:
 ...
 *
 * Usage: Run manually after reviewing Production sheet, or via daily trigger (4am UTC).
 */
...
// Plain spreadsheet ID from the LIVE sheet's /edit URL:
//   https://docs.google.com/spreadsheets/d/SPREADSHEET_ID/edit
// NOT the published CSV URL — those are different IDs.
// Published CSV URL (for D1 sync worker reference):
...
const LIVE_SHEET_ID = '1_05QcI1y4YuWyzGboncIRffyHoDan6a2_RJFrYqh2OA';
const LIVE_SHEET_NAME = 'LIVE';      // Tab name in the LIVE spreadsheet
...
function syncProductionToLive() {
  Logger.log('=== SYNCING PRODUCTION → LIVE (JAMAICA) ===\n');
