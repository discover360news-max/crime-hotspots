/**
 * RSS Feed Collector for Jamaica Crime News
 ...
 * Trigger: Runs 3x daily via time-based trigger (see orchestrator.gs setupAllTriggers)
 ...
 * References: NEWS_SOURCES from config.gs (single source of truth — 4 Jamaica feeds)
 *
 * Last Updated: 2026-03-17 (adapted for Jamaica)
 */
...
function fetchAndParseFeed(feed) {
  ...
  return items.map(item => {
    ...
    if (!pubDate) {
      ...
      pubDate = Utilities.formatDate(new Date(), 'America/Jamaica', "yyyy-MM-dd'T'HH:mm:ss");
