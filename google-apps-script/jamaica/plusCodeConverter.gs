...
// Jamaica center for context when needed
const JAMAICA_LAT = 17.9970;
const JAMAICA_LNG = -76.7936;
...
function decodePlusCodeWithGoogleAPI(plusCodeString) {
  try {
    ...
    // Use Google Geocoding API with region biasing for Jamaica
    // The 'components' parameter restricts results to Jamaica
    // For very short codes, the API may struggle, but country restriction helps
    const url = `https://maps.googleapis.com/maps/api/geocode/json?address=${encodeURIComponent(plusCode)}&components=country:JM&region=jm&key=${apiKey}`;
    ...
    if (data.status === 'OK' && data.results && data.results.length > 0) {
      ...
      // Verify the result is actually in Jamaica (lat ~18, lng ~-77)
      if (Math.abs(location.lat - JAMAICA_LAT) < 5 && Math.abs(location.lng - JAMAICA_LNG) < 5) {
        return {
          success: true,
          lat: location.lat,
          lng: location.lng
        ...
        return {
          success: false,
          error: 'Result outside Jamaica area'
        };
      }
