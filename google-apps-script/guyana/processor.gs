 ...
 * Returns formatted string to prevent Google Sheets auto-reformatting based on locale
...
function validateAndFormatDate(dateStr, fallbackDate) {
  ...
  const day = Utilities.formatDate(dateToFormat, Session.getScriptTimeZone(), 'dd');
  const month = Utilities.formatDate(dateToFormat, Session.getScriptTimeZone(), 'MM');
  const year = Utilities.formatDate(dateToFormat, Session.getScriptTimeZone(), 'yyyy');
...
/**
 * Normalize URLs for comparison (handles news source URL variations)
 * Some news sources use UUIDs or IDs in URLs where the slug can vary
 *
 * @param {string} url - Source URL
 * @returns {string} Normalized URL (article ID where applicable, original URL otherwise)
 */
function normalizeUrl(url) {
  if (!url) return url;

  // Trinidad Express pattern (if Guyana sources cross-reference): Extract article ID
  const expressMatch = url.match(/article_([a-f0-9-]+)\.html/i);
  if (expressMatch) {
    return `article:${expressMatch[0]}`; // Returns "article:article_UUID.html"
  }

  // Add other patterns as needed for Guyana news sources
  // Example: if Kaieteur News uses ?p=12345 format
...
function isDuplicateCrime(sheet, crime, geocoded) {
    ...
    for (let row of data) {
      ...
      // Catches when location names appear in different field combinations
      // ═══════════════════════════════════════════════════════════
      if (existingDate && crime.crime_date) {
        const existingDateStr = Utilities.formatDate(new Date(existingDate), Session.getScriptTimeZone(), 'yyyy-MM-dd');
...
function extractNamesFromHeadline(headline) {
  ...
  if (capitalizedMatches) {
    capitalizedMatches.forEach(match => {
      ...
      const blacklist = ['Home Invasion', 'Main Street', 'Camp Street', 'Stabroek Market', 'Georgetown', 'New Amsterdam'];
...
function checkBacklogStatus() {
  Logger.log('=== CHECKING BACKLOG STATUS (GUYANA) ===');
  ...
  if (readyCount > BACKLOG_ALERT_THRESHOLD) {
    ...
    const emailBody = `
⚠️ Large Processing Backlog Detected (GUYANA)
...
View spreadsheet: ${SpreadsheetApp.getActiveSpreadsheet().getUrl()}
    ...
    GmailApp.sendEmail(
      NOTIFICATION_EMAIL,
      `⚠️ Crime Hotspots GUYANA - Large Backlog (${readyCount} articles pending)`,
      emailBody
    );
