/**
 * RSS Feed Collector for Barbados Crime News
 ...
 * References: NEWS_SOURCES from config.gs (single source of truth)
 *
...
function collectAllFeeds() {
  ...
  // Use NEWS_SOURCES from config.gs (single source of truth)
...
function fetchAndParseFeed(feed) {
  ...
  return items.map(item => {
    return {
      ...
      fullText: '' // Will be fetched later by articleFetcherImproved.gs
    };
  });
}
...
function testEditorialFilter() {
  ...
  const testArticles = [
    // Should be FILTERED
    {title: 'Commentary: Crime is rising in Barbados', url: 'http://barbadostoday.bb/test1', description: '', expected: true},
    {title: 'Opinion: We need better policing', url: 'http://nationnews.com/test2', description: '', expected: true},
    {title: 'Letter to Editor: My thoughts on crime', url: 'http://barbadostoday.bb/test3', description: '', expected: true},
    ...
    // Should be ALLOWED
    {title: 'Man shot in Bridgetown', url: 'http://barbadostoday.bb/news/test6', description: '', expected: false},
    {title: 'Police investigate robbery in St. Michael', url: 'http://nationnews.com/news/test7', description: '', expected: false},
    {title: 'Three arrested in drug bust', url: 'http://barbadosunderground.net/test8', description: '', expected: false},
    {title: 'Commissioner discusses crime trends', url: 'http://barbadostoday.bb/test9', description: '', expected: false}
  ];
...
function testRSSCollection() {
  Logger.log('═════════════════════════════');
  Logger.log('Starting RSS Collection Test (Barbados)');
  Logger.log('═════════════════════════════');
