...
export function generateRelatedCrimesHTML(currentCrime: any, allCrimes: any[], maxCards: number = 4): string {
  ...
  const crimeCards = relatedCrimes.map((crime: any) => {
    ...
    return `
      <a href="${escapeHtml(crimeUrl)}" data-related-crime-slug="${escapeHtml(crime.slug)}" class="related-crime-link block p-3 bg-white rounded-lg border border-slate-100 hover:border-rose-200 hover:shadow-sm transition-all group">
        <div class="flex items-start gap-2">
          ...
          <div class="flex-1 min-w-0">
            ...
            <div class="flex items-center gap-2 mt-1">
              <span class="text-[10px] text-slate-400">${formatDate(crime.date)}</span>
              ${isSameArea
                ? '<span class="text-[10px] px-1.5 py-0.5 bg-rose-50 text-rose-600 rounded">Same area</span>'
                : `<span class="text-[10px] text-slate-400 truncate">${escapeHtml(crime.area || '')}</span>`
              }
            </div>
          </div>
  ...
  return `
    <aside class="p-4 bg-slate-50/80 backdrop-blur-sm rounded-lg border border-slate-200">
      ...
      <a href="/trinidad/area/${escapeHtml(areaSlug)}/" class="mt-3 flex items-center justify-center gap-1 w-full py-2 text-xs font-medium text-rose-600 hover:text-rose-700 hover:bg-rose-50 rounded-lg transition-colors">
